...
작성일: 2026-09-15 · 갱신 2026-09-22 · 상태: 서버 준비 완료 · 화면 골격 + 3건 보완 완료 · 나머지 화면 미착수
...
cd web; npm ci; npm run dev                                           # 화면 :3000
```
...
| 함정 | 이유 |
|---|---|
| `http://127.0.0.1:3000`으로 열면 지도가 안 뜬다 | 카카오 콘솔에 `http://localhost:3000`만 등록돼 있다. 같은 키로 `localhost:3000`은 200, `127.0.0.1:3000`·`localhost:3001`은 **401 domain mismatched**다. 편하게 하려면 콘솔 플랫폼 > Web에 `http://127.0.0.1:3000`을 추가한다 |
| 3000 포트가 이미 쓰이면 Next가 3001로 뜬다 | 그 포트도 등록돼 있지 않아 401이다. 기존 프로세스를 끄고 3000으로 띄운다 |
| 8000 포트에 임시 데모 서버가 떠 있으면 화면이 가짜 자료를 본다 | 2026-09-22에 `demo A`·`demo B`를 주던 Node 스텁을 발견해 껐다. 실제 API인지 확인: `curl localhost:8000/healthz` → `{"status":"ok"}` |
| 키는 `web/.env.local`에 있다 | `NEXT_PUBLIC_KAKAO_MAP_JS_KEY`. 이 파일은 git에 올라가지 않는다. 바꾸면 `npm run dev`를 다시 시작한다 |

### 지금 화면에 보여야 정상인 것

실제 DB 상태(2026-09-22): 노선 4개 · 시간표 회차 148개 · 날짜별 회차 933개 · 정거장 22개 · **좌표 0개** · 관측 0건 · 조사 트랙 0건.

- 정거장 목록 22개가 **전부 '좌표 확인 필요'**
- **마커 0개** — 좌표가 없으면 마커를 만들지 않는 것이 규칙이다 (`../md/10` 3장)
- 지도 배경(타일)은 떠야 한다. 안 뜨면 위 함정표의 주소 문제를 먼저 본다
- 예상 시각은 공시 시간표 기준이다. 실측 관측이 0이라 '실시간'이 아니다

좌표를 넣어 마커를 보고 싶으면 서버 담당에게 `api/samples/stops-provisional.json` 적재를 요청한다. 전부 `needs_interpretation`으로 들어가 '확인 필요' 마커로 표시된다. 화면 시험을 위해 실제 DB에 `verified`를 만들지 않는다.

### 검증된 것과 아닌 것

| 항목 | 상태 |
|---|---|
| `npm ci`·타입·lint·회귀 시험 6개·`audit`(0건)·프로덕션 빌드 | 2026-09-22 재확인 통과 |
| 카카오 SDK 경로 | 키·도메인·네트워크 확인 (`sdk.js` → 지도 본체 → 타일 모두 200) |
| 실제 지도 화면 | **사용자 브라우저 확인 대기** |
| Docker로 화면 띄우기 | **미검증** — `web` 서비스는 Compose에 있으나 빌드 인자(`NEXT_PUBLIC_*`)를 루트 `.env`에 넣어야 한다 |
...
2026-09-22 후속: 미검증 좌표 마커 구분·경로 요청 취소·Next 16 취약점 보완 세 건은 **구현 완료**. 자동 검사와 데모 화면 확인 결과, 실제 카카오 지도·Docker 검증의 남은 조건은 [화면 3건 수정 계획·결과](PLAN-screen-fixes.md)에 기록했다.

| # | 할 일 | 근거 |
|---|---|---|
| F1 | **골격 + 보완 3건 완료 (2026-09-22)** — 지도·정거장 목록·카드·노선/날짜 선택, 미검증 좌표 마커 구분, 요청 경쟁 방지, Next 16. **남은 화면**: 검색(출발·도착 후보), 시간표 메뉴, 왼쪽 햄버거 패널, 프로필 설정, 공지 배너 | [01](01-screens.md), [03](03-figma-handoff.md), [PLAN-screen-fixes](PLAN-screen-fixes.md), ../web |
...
| F8 | **구현 (2026-09-22)** — 정거장 점만, 직선 연결 없음, SDK 실패 시 목록으로 진행. 확인 좌표는 파란 원, 미검증은 주황 삼각 마커 + 범례. **정거장 좌표는 여전히 0/22** — 등록 전에는 마커가 하나도 없다 | ../md/10-stop-discovery.md 2·3장 |
