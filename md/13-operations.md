...
- 회차 취소는 이미 완료된 슬롯의 `completed`를 보존하고 나머지 슬롯을 `cancelled`로 만든다. 회차 자체는 `cancelled`가 된다 (운영 구현 메모 및 현재 코드 기준).
- 이미 공개한 회차를 취소하면 **사유와 상태 변경을 전송한다** (`12`).
- 학생 화면은 결정 트리 1번에서 "운행 취소"와 사유를 표시한다 (`03` 6장).
...
| FR-OP-06 | 회차 취소가 미완료 차량을 취소 | 취소 후 미완료 슬롯 cancelled·완료 슬롯 completed 유지 확인 |
