...
> 작성 2026-09-18 · 상태 갱신 2026-09-22 (적재기·경로 API 구현·시험 통과, 실제 자료는 여전히 없음). 현장 자료(정거장 좌표·GPX·GPS 단말)가 아직 없는 동안 **가짜 자료로 파이프라인 전체를 먼저 돌려 보고**, 진짜 자료가 오면 같은 명령에 넣기만 하면 되게 만든다. 규칙은 `04`·`07`·`10`이 소유하고 이 문서는 순서와 완료 판정만 정한다. 화면(FE)은 외부에 맡기므로 여기서는 `web/`에 경로선 표시 최소만 붙인다.
