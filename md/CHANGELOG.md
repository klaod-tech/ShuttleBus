...
| 화면 의존성 | 잠금 파일 유지·Next 16.3.5 상향으로 PostCSS 경유 취약점 해소. 상세 검증은 위 화면 3건 구현 기록 참조 |
...
화면 후속 3건은 같은 날 사용자 승인으로 구현했다. 실제 카카오 지도·Docker 검증은 위 화면 구현 결과를 따른다.

### 화면 3건 구현과 재확인

| 무엇 | 어떻게 |
|---|---|
| 미검증 좌표 구분 | 확인 좌표는 파란 원, 미검증은 주황 삼각 로컬 SVG 마커. 마커 제목·범례·목록 문구에 '좌표 확인 필요'. 좌표는 위도·경도 양쪽을 본다 |
| 경로·정거장 요청 경쟁 | 성공·실패 콜백이 모두 `signal.aborted`를 확인한 뒤 상태를 바꾼다. 노선·날짜를 빠르게 바꿔도 이전 응답이 최신 선택을 덮지 않는다 |
| 의존성 취약점 | Next **16.3.5**로 올리고 잠금 파일 갱신. `lint`는 ESLint CLI, `test`는 Vitest/jsdom. `npm audit` 취약점 0건 |
| 검증 (다른 세션 재확인) | `npm ci`·타입·lint·시험 6개·audit 0건·프로덕션 빌드 통과 |
| **카카오 SDK 실패 원인** | 키·도메인 등록 문제가 아니었다. 같은 키로 `localhost:3000`은 200, `127.0.0.1:3000`·`localhost:3001`은 **401 domain mismatched**다. 개발 서버가 `127.0.0.1:3000`에만 바인딩되므로 `http://localhost:3000`으로 열어야 한다 ([계획 문서](../md_frontend/PLAN-screen-fixes.md)) |
| 브라우저 검증 환경 | 8000 포트의 임시 데모 서버를 끄고 실제 API로 바꿨다. 실제 노선 4개·CORS 헤더 확인. 화면에 보이는 자료가 가짜가 아니게 했다 |
| 문헌 기록 | 추가 질문 경로가 없어져, [논문.md](논문.md)의 '교수님께 남은 질문'을 **우리가 스스로 정할 것**으로 바꿨다 — 단말·측정 주기·갱신 기준·신호 정보·평가 기준 자료 |
